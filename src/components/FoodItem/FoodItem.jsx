import React, { useContext } from 'react'
import "./FoodItem.css";
import assets from '../../assets/assets';
import { StoreContext } from '../../context/StoreContext';

export const FoodItem = ({id,name,price,description,image}) => {

const {cartItems,addToCart,removeFromCart,url} = useContext(StoreContext);
  return (
    <div className='food-item'> 
        <div className="food-item-img-container">
           <img  className="food-item-image" src={url+"/images/"+image} alt=""/>
           {!cartItems[id]
           ?<img className='add' onClick={()=>addToCart(id)}  src={assets.add_icon_white} alt="" style={{ width: '24px', height: '24px', objectFit: 'contain' }}/>
           :<div className='food-item-Counter'>
            <img onClick={()=>removeFromCart(id)} src={assets.remove_icon_red} alt="" style={{ width: '24px', height: '24px', objectFit: 'contain' }}/>
            <p>{cartItems[id]}</p>
            <img onClick={()=>addToCart(id)} src={assets.add_icon_green} alt="" style={{ width: '24px', height: '24px', objectFit: 'contain' }}/>
            </div> } 
           </div>
           <div>
            </div>
      <div className="food-item-info">
        <div className="food-item-name-rating">
            <p>{name}</p>
           <img src={assets.rating } alt=""  style={{ width: '24px', height: '24px', objectFit: 'contain' }}/> 
           </div>
            
            <p className="food-item-desc">{description}</p>
            <p className="food-item-price">${price}</p>
      
    </div>
    </div>
  )
}

export default FoodItem;
